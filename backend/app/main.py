...
app = FastAPI(
    ...
    version="1.0.0",
    description="Deterministic static analysis APIs for code, reasoning, rubrics, and LLM-code evaluation.",
)
...
def bugs_endpoint(request: CodeRequest) -> BugDetectorResponse:
    ...
    record("Bug Detector", severity_score, request, [finding.category for finding in response.findings])
    return response
...
def complexity_endpoint(request: CodeRequest) -> ComplexityResponse:
    ...
    record("Complexity Analyzer", response.confidence_score, request, complexity=response.time_complexity)
    return response
