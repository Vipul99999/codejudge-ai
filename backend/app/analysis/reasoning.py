...
def review_reasoning(request: ReasoningRequest) -> ReasoningResponse:
    ...
    connectors = sum(lower.count(word) for word in ["because", "therefore", "so", "however", "assume", "if", "then"])
    contradictions = []
    if "always" in lower and "except" in lower:
        contradictions.append("Uses absolute language and an exception in the same reasoning chain.")
    if "must" in lower and "might" in lower:
        contradictions.append("Mixes certainty with uncertainty without explaining the condition change.")
    missing = []
    ...
    score = clamp(52 + min(steps, 6) * 5 + min(connectors, 10) * 3 - len(contradictions) * 15 - len(missing) * 7)
    feedback = [
    ...
    if score < 70:
        feedback.append("Add a clearer chain from assumptions to conclusion and test it against counterexamples.")

    return ReasoningResponse(
        score=score,
        feedback=feedback,
        missing_assumptions=missing or ["No major missing assumption detected by heuristic review."],
