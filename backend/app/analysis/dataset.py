...
def build_dataset(request: DatasetRequest) -> DatasetResponse:
    tags = request.tags or [request.topic.lower().replace(" ", "-"), "static-analysis", "evaluation"]
    ...
    for index in range(request.count):
        ...
        problem = f"{request.topic}: evaluate candidate solution variant {index + 1} and identify correctness, complexity, and edge-case risks."
        solution = (
            f"Reference evaluation note {seed}: inspect invariants, boundary behavior, and complexity. "
        ...
        items.append(
            DatasetItem(problem=problem, solution=solution, test_cases=cases, difficulty=difficulty, tags=tags)
        )
    ...
    for item in items:
        csv_lines.append(
            ",".join(
                [csv_escape(item.problem), csv_escape(item.solution), item.difficulty, csv_escape("|".join(item.tags))]
            )
        )
