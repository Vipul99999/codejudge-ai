...
LANGUAGE_HINTS: dict[Language, dict[str, str]] = {
    ...
    Language.java: {"comment": "//", "function": r"\b(public|private|protected)?\s*(static\s+)?[\w<>\[\]]+\s+\w+\s*\("},
...
def python_ast_signals(code: str) -> tuple[bool, str | None, int, int, int, int, list[str]]:
    try:
        tree = ast.parse(code)
    except SyntaxError as exc:
        return False, f"Python syntax error near line {exc.lineno}: {exc.msg}", 0, 0, 0, 0, []
    ...
    def depth(node: ast.AST, current: int = 0) -> int:
        ...
        bump = 1 if isinstance(node, (ast.If, ast.For, ast.While, ast.Try, ast.With, ast.Match)) else 0
        if not children:
            return current + bump
    ...
    return True, None, len(functions), len(loops), len(branches), depth(tree), identifiers, long_functions
...
def collect_signals(code: str, language: Language) -> CodeSignals:
    ...
    normalized = [line for line in non_empty if not line.startswith(comment_prefix) and len(line) > 6]
    ...
    if language == Language.python:
        ...
        parse_ok, parse_error, function_count, loop_count, branch_count, nesting_depth, py_ids, long_functions = py
...
def confidence_from_signals(signals: CodeSignals, language: Language) -> tuple[Confidence, str, int]:
