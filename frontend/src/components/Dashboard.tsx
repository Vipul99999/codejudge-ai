...
function flatten(records: Record<string, number>[]) {
  return records.flatMap((record) => Object.entries(record).map(([name, value]) => ({ name, value })));
}
          ...
          <Card key={String(title)}>
            ...
            <CardContent className="h-72">
              {Array.isArray(data) && data.length ? (
                ...
                <div className="flex h-full items-center justify-center rounded-md border border-dashed border-line text-sm text-slate-500">No data yet</div>
              )}
            </CardContent>
          </Card>
        ))}
