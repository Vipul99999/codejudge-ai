...
function extractScore(result: Record<string, unknown>) {
  const score = result.overall_score ?? result.score ?? result.weighted_score ?? result.confidence_score;
  return typeof score === "number" ? Math.round(score) : undefined;
}

export default function Home() {
  const { activeModule, setActiveModule, language, setLanguage, history, addHistory } = useEvaluationStore();
  const [showDashboard, setShowDashboard] = useState(false);
  const [code, setCode] = useState(sampleCode);
  const [solutionB, setSolutionB] = useState(sampleCodeB);
  const [prompt, setPrompt] = useState("Write a robust two-sum implementation and explain edge cases.");
  const [aiResponse, setAiResponse] = useState("The function uses a hash map, handles missing pairs by returning an empty list, and runs in O(n) time.");
  const [problem, setProblem] = useState("Evaluate whether a two-sum solution is correct for duplicates and empty arrays.");
  const [reasoning, setReasoning] = useState("1. Assume nums is a finite list.\n2. Track seen values because complements identify a pair.\n3. Empty input returns no pair.\n4. Complexity is O(n).");
  ...
  const active = useMemo(() => modules.find((module) => module.key === activeModule) ?? modules[0], [activeModule]);
  ...
  function payloadFor(module: ModuleKey) {
    ...
    if (module === "dataset") return { topic: datasetTopic, language, count: 6, difficulty: "mixed", tags: ["ai-evaluation", "benchmarking"] };
        ...
        <div className="mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-4 px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Professional static evaluation for code, prompts, reasoning, and datasets.</p>
            </div>
          </div>
                ...
                <div className="grid gap-4">
                  {activeModule === "llm-evaluate" ? (
                    <Card>
                      ...
                      <CardContent className="grid gap-3">
                        ...
                        <Textarea value={aiResponse} onChange={(event) => setAiResponse(event.target.value)} aria-label="AI Response" className="min-h-52" />
                      </CardContent>
                    </Card>
                  ) : null}
                  ...
                  {activeModule === "reasoning" ? (
                    <Card>
                      ...
                      <CardContent className="grid gap-3">
                        <Textarea value={problem} onChange={(event) => setProblem(event.target.value)} aria-label="Problem" />
                        <Textarea value={reasoning} onChange={(event) => setReasoning(event.target.value)} aria-label="Reasoning" className="min-h-60" />
                      </CardContent>
                    </Card>
                  ) : null}
                  ...
                  {activeModule === "dataset" ? (
                    <Card>
                      ...
                      <CardContent>
                        <Textarea value={datasetTopic} onChange={(event) => setDatasetTopic(event.target.value)} aria-label="Dataset Topic" />
                      </CardContent>
                    </Card>
                  ) : null}
                  ...
                  {activeModule === "rubric" ? (
                    <Card>
                      ...
                      <CardContent>
                        <Textarea value={rubric} onChange={(event) => setRubric(event.target.value)} aria-label="Rubric JSON" className="min-h-80 font-mono" />
                      </CardContent>
                    </Card>
                  ) : null}
                  ...
                  {!["llm-evaluate", "reasoning", "dataset", "rubric"].includes(activeModule) ? (
                    <Card>
                      <CardHeader>
                        <CardTitle>{activeModule === "compare" ? "Solution A" : "Submission"}</CardTitle>
                      </CardHeader>
                      <CardContent>
                    ...
                    <CardContent className="grid gap-2">
                      {history.length ? (
                        history.slice(0, 6).map((entry) => (
                          <div key={entry.id} className="flex items-center justify-between gap-3 rounded-md border border-line bg-slate-50 px-3 py-2 text-sm">
