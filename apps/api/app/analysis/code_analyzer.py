...
def analyze_code(request: CodeRequest) -> CodeAnalysisResponse:
    ...
    readability = clamp(
        92
        - max(0, signals.avg_line_length - 80) * 0.45
        - signals.nesting_depth * 5
        - short_name_ratio * 18
    )
    maintainability = clamp(
        88
        - signals.long_functions * 12
        - signals.duplicate_lines * 4
        - max(0, signals.function_count - 12) * 2
    )
    complexity = clamp(
        94 - signals.loop_count * 6 - signals.branch_count * 4 - signals.nesting_depth * 7
    )
    naming = clamp(90 - short_name_ratio * 36 - max(0, 0.05 - comment_ratio) * 70)
    smells = clamp(
        94
        - signals.duplicate_lines * 7
        - signals.long_functions * 15
        - (0 if signals.parse_ok else 25)
    )
    edge_case = clamp(
        50
    ...
    categories = [
        category(
            "Readability", readability, "Based on line length, nesting, and identifier clarity."
        ),
        category(
            "Maintainability",
            maintainability,
            "Penalizes duplication, long functions, and overly broad modules.",
        ),
        category(
            "Complexity",
            complexity,
            "Estimates control-flow burden from loops, branches, and nesting.",
        ),
        category(
            "Naming Quality",
            naming,
            "Looks for descriptive identifiers and useful local documentation signals.",
        ),
        category(
            "Code Smells", smells, "Flags duplication, parse failures, and large function bodies."
        ),
        category(
            "Edge Case Coverage",
            edge_case,
            "Checks whether boundary and empty-input handling is visible in code.",
        ),
    ]
    ...
    if signals.max_line_length > 120:
        suggestions.append(
            "Break very long lines into named intermediate values or helper functions."
        )
    ...
    if edge_case < 65:
        suggestions.append(
            "Add explicit handling for empty inputs, null values, boundaries, and invalid states."
        )
    ...
    if not suggestions:
        suggestions.append("Clean static profile; focus next on runtime tests and peer review.")
