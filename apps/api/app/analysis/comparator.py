...
def compare_solutions(request: CompareRequest) -> ComparisonResponse:
    ...
    complexity_a = analyze_complexity(
        CodeRequest(language=request.language, code=request.solution_a)
    )
    complexity_b = analyze_complexity(
        CodeRequest(language=request.language, code=request.solution_b)
    )
    ...
    breakdown = [
        category(
            ...
            safe_average(
                [analysis_a.category_scores[-1].score, analysis_b.category_scores[-1].score]
            ),
        ...
        category(
            "Readability",
            safe_average(
                [analysis_a.category_scores[0].score, analysis_b.category_scores[0].score]
            ),
        ...
        category(
            "Scalability",
            safe_average([efficiency_a, efficiency_b]),
            "Based on estimated asymptotic growth.",
        ),
    ]
